...
| 숫자와 한글 조사도 붙여 쓴다 | `글줄이 20px 이라` | `글줄이 20px이라` |
| 괄호 · 따옴표 · 딱지로 감싼 말도 마찬가지다 | `[편집] 을 누르면` | `[편집]을 누르면` |
...
- 제목 — 커밋 제목과 같다. `index.css`의 절 제목처럼 종결형으로 끝나도 마침표를 찍지 않는다.
- 이어지는 줄 — 한 문장이 다음 줄로 넘어가면 그 줄 끝에는 찍지 않는다.
- 자리를 맞춘 표 · 그림 — 칸을 나누느라 둔 공백은 줄이지 않는다.
- 화면에 그대로 보이는 이름 — `(...)` · `(영역)`처럼 괄호까지가 이름이면 앞말에 붙이지 않는다.
- 의존명사 — `걷는 만큼` · `넓어질 뿐` · `이름일 뿐`처럼 조사가 아닌 것은 띄운다.
