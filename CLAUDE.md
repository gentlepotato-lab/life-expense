...
- **요청하지 않은 기능·레이아웃·자산을 바꾸지 않는다.** 손대지 않은 UI 배치, 문구, 동작은
  불변으로 취급한다.
- 요구사항·데이터 계약·UX에 추측이 필요하면 먼저 질문한다.
...
**저장소를 변경하는 Git 명령은 사용자가 직접 실행한다. Claude는 실행하지 않는다.**
...
문서(`.md`) · 응답 본문 · **화면에 보이는 문구** · **코드 주석** ·
**커밋 메시지(제목과 본문)** 모두에 적용한다.

| 규칙 | ✗ | ✓ |
|---|---|---|
| 여는 괄호는 앞말에 붙인다 | `구성 (2026-08-09)` | `구성(2026-08-09)` |
| 영어 단어와 한글 조사는 붙여 쓴다 | `lab.ps1 로만 띄운다` | `lab.ps1로만 띄운다` |
| 종결형 문장에는 마침표를 찍는다 | `값을 바꾼다` | `값을 바꾼다.` |
| 영어 단어는 첫 글자를 대문자로 쓴다 | `nginx 재기동` | `Nginx 재기동` |
| 하이픈으로 연결된 영어 단어는 각 단어의 첫 글자를 대문자로 쓴다 | `bind-mount 방식` | `Bind-Mount 방식` |

**마침표는 종결형에만 찍는다.** 명사나 부사로 끝나는 줄에는 찍지 않는다 —
한 목록 안에 두 가지가 뒤섞여 있어도 그렇다.

```
- 잔소리에 목표 규칙 셋 — 넘겼다 · 이대로면 넘긴다 · 달 후반에만 남았다.
- 이번 달 현황과 꾹 눌러 보는 내역 상세
- 설정 탭 입력칸 모서리를 8px로 맞추고 안내 글씨를 한 단 옅게
```
...
- 코드 자체 — `if (`, `=> (`, `function (`처럼 문법으로 정해진 괄호
- 수식 표기 — `5 × (3 + 2)`처럼 연산자 뒤에 오는 괄호
- 날짜 뒤의 요일 — `2026. 8. 18. (토)`처럼 마침표로 끝난 뒤의 괄호는 띄운다.
- 커밋 제목 — `feat: 안쓴이 도전 추가`처럼 명사로 끝나므로 마침표를 찍지 않는다.
...
| 실서비스 | 게이트웨이 Nginx가 `frontend/dist`를 **직접** 서빙(`:8101`, `expense.life.localhost`) |

### 경로 규칙

- **API는 모두 `/api` 아래.** `app/main.py`의 `API = "/api"` 한 곳에서 붙인다.
...
- **SPA는 루트.** `BrowserRouter`에 `basename` 없음, `vite.config.ts`는 `base: '/'`.
  API가 `/api`로 비켜서면서 충돌이 사라졌다(2026-08 이전에는 `/app/`에 있었다).
- 예전 `/app/…` 주소는 게이트웨이(`Gateway\nginx\conf\conf.d\20-life.conf`)가 301로 넘긴다.
...
- **Tailwind는 쓰지 않는다.** 유틸리티처럼 보이는 클래스명은 프로젝트 자체 CSS 다.
